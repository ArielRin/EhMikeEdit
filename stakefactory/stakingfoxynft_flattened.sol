...
// File: @openzeppelin/contracts@4.5.0/token/ERC721/utils/ERC721Holder.sol
...
// File: foxynftstaking/IBEP20.sol
...
// File: foxynftstaking/IRewardPool.sol
...
// File: foxynftstaking/stakingfoxynft.sol
...
contract NftRewardPoolV2 is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    // Platform Tax applied on registration
    uint256 public registrationFee;
    address public registrationFeeAddress;
    ...
    constructor(RewardPoolConfiguration memory config) payable {
        _initializePool(config);
        if (msg.value > 0)
            payable(config.admin).transfer(msg.value);
    }
     ...
     */
    function _initializePool(RewardPoolConfiguration memory config) private {
        require(config.tax == 0 || config.taxAddress != address(0), "tax recipient");
        require(config.admin != address(0), "admin");
        ...
        registrationFee = config.tax;
        registrationFeeAddress = config.taxAddress;
    ...
    function _register(address account) internal {
        require(block.number >= startBlock, "Registration is closed");
        require(!userInfo[account].registered, "Already registered");
        require(msg.value >= registrationFee, "Invalid payment value");
        userInfo[account].registered = true;
        payable(registrationFeeAddress).transfer(msg.value);
    ...
    function deposit(uint256[] calldata tokenIds) external payable nonReentrant {
        ...
        uint256 userTotalStaked = user.tokens.length();

        if (registrationFee > 0 && !user.registered) {
            _register(msg.sender);
        }

        if (hasUserLimit) {
            require(tokenIds.length.add(userTotalStaked) <= poolLimitPerUser, "limit");
        }

        _updatePool();

        if (userTotalStaked > 0) {
            uint256 pending = userTotalStaked.mul(TOKEN_MULTIPLIER).mul(accTokenPerShare).div(PRECISION_FACTOR).sub(
                user.rewardDebt
            );
        ...
        if (tokenIds.length > 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                user.tokens.add(tokenIds[i]);
                stakedToken.safeTransferFrom(msg.sender, address(this), tokenIds[i], "");
            }
        }

        user.rewardDebt = user.tokens.length().mul(TOKEN_MULTIPLIER).mul(accTokenPerShare).div(PRECISION_FACTOR);
    ...
    function withdraw(uint256[] calldata tokenIds) external nonReentrant {
        ...
        uint256 pending = user.tokens.length().mul(TOKEN_MULTIPLIER).mul(accTokenPerShare).div(PRECISION_FACTOR).sub(
            user.rewardDebt
        );
        ...
        if (tokenIds.length > 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                ...
                require(user.tokens.contains(tokenId), "Not owner");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            return
                user.tokens.length().mul(TOKEN_MULTIPLIER).mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(
                    user.rewardDebt
                );
        } else {
            return
                user.tokens.length().mul(TOKEN_MULTIPLIER).mul(accTokenPerShare).div(PRECISION_FACTOR).sub(
                    user.rewardDebt
                );
        }
    }
    ...
    function isRegistered(address account) public view returns (bool) {
        return userInfo[account].registered;
    }
    ...
    function getUserStakedTokensCount(address account) public view returns (uint256) {
        return userInfo[account].tokens.length();
    }
    ...
    function getUserStakedTokens(
        ...
        uint256 length = size;
        if (length > userInfo[account].tokens.length() - cursor) {
            length = userInfo[account].tokens.length() - cursor;
        }

        uint256[] memory values = new uint256[](length);
        for (uint256 i = 0; i < length; i++) {
            values[i] = userInfo[account].tokens.at(cursor + i);
        }
    ...
    function vaultConfiguration()
        ...
        returns (
            ...
            uint256 endDepositBlock,
            uint256 maxTokensStaked,
            uint256 maxTokensPerUser,
            uint256 vestDuration
        )
    {
        return (registrationFee > 0, startBlock, bonusEndBlock, 0, poolLimitPerUser, 0);
    }
